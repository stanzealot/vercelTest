export function emailVerificationView(token: string): string {
  ...
  let temp = `
     ...
     <h2 style="text-align: center; text-transform: uppercase;color: teal;">Welcome to MoniWizr.</h2>
...
export function forgotPasswordVerification(token: string): string {
  const link = `${process.env.FRONTEND_URL}/user/reset-password/${token}`;
...
export function generalAuthEmail(path: string): string {
  const link = `${path}`;
  let temp = `
     <div style="max-width: 700px;
     margin:auto; border: 10px solid #ddd; padding: 50px 20px; font-size: 110%;"
